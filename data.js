  ...
  topics: [
      ...
      problems: [
        {
          ...
          approach: "I traverse the list and store each node object in a set. If I ever visit a node that's already in the set, a cycle exists.",
          insight: "💡 The O(1) space solution uses Floyd's Tortoise and Hare, two pointers at different speeds. If there's a cycle, the fast pointer eventually laps the slow one. My solution trades that space efficiency for simplicity.",
          code: `class Solution:
    ...
    def detectCycle(self, head):
        ...
        },
        {
          ...
          insight: "💡 This is a linked list operation. Mastering it makes problems like Palindrome Linked List (234) much more approachable, since reversing half the list is a key sub-step.",
          code: `class Solution:
    def reverseList(self, head):
        ...
        },
        {
          ...
          insight: "💡 This problem efficiently combines two patterns, slow/fast pointer midpoint finding and in-place list reversal. It took me 5 attempts because I initially struggled with the pointer state after reversal. Getting the order of operations right was the key lesson.",
          code: `class Solution:
    def isPalindrome(self, head):
        ...
        },
        {
          ...
          insight: "💡 The dummy node pattern removes the special case of initializing the head of the result list, making the loop cleaner and less error-prone.",
          code: `class Solution:
    ...
    def isSymmetric(self, root):
        ...
        },
        {
          ...
          time: "O(n)", space: "O(h)", technique: "Recursive DFS",
          approach: "The depth of any node is 1 plus the max depth of its two children. The base case is null. This naturally expresses as a single recursive return statement.",
    ...
    def maxDepth(self, root):
        ...
        },
        {
          ...
          approach: "If the target is smaller go left, if larger go right, and return when found. Each move cuts the search space in half.",
          insight: "💡 In a balanced BST, searching takes O(log n) time. This works because the tree is sorted, so each step lets you ignore about half of the remaining nodes, similar to binary search.",
          code: `class Solution:
    def searchBST(self, root, val):
        ...
        },
        {
          ...
          insight: "💡 Passing a running value through DFS keeps track of the result as you go, so you don’t need extra steps to undo work or store full paths. The function calls naturally remember the current state.",
          code: `class Solution:
      ...
      problems: [
        {
          ...
          insight: "💡 This connects to tries because tries are designed to store strings so that shared prefixes are reused, making prefix-based searches fast and efficient.",
          code: `class Solution:
...
class Trie:
      ...
      problems: [
        {
          ...
          insight: "💡 One could also use a min-heap of size k to keep track of the k largest elements efficiently, since you only need to store the best k values at any time. This makes updates faster than keeping everything sorted.",
    ...
    def add(self, val):
        ...
        },
        {
          ...
          insight: "💡 Python’s heapq is a min-heap. If we keep its size at k, we always remove smaller values so only the k largest stay. The smallest value in the heap (the root) is then the kth largest overall.",
          code: `import heapq
class Solution:
      ...
      problems: [
        {
          ...
          insight: "💡 Instead of checking all pairs, we store numbers we’ve seen in a hash map so we can quickly find if the matching value exists. This turns a slow O(n²) approach into an O(n) one.",
          code: `class Solution:
    def twoSum(self, nums, target):
        ...
        },
        {
          ...
          insight: "💡 Merging from right to left avoids overwriting values. If you merge left to right, you’d have to shift elements. Starting from the right lets you fill empty space safely without moving existing elements.",
          code: `class Solution:
    def merge(self, nums1, m, nums2, n):
        ...
        },
        {
          ...
          insight: "💡 Storing the index in a hash map lets us move the left pointer straight to the correct position instead of stepping forward one by one. This keeps the solution efficient at O(n).",
          code: `class Solution:
      ...
      problems: [
        {
          ...
          insight: "💡 This problem and #232 together illustrate the conceptual difference between stacks and queues by forcing you to build one from the other.",
          code: `class MyStack:
    def __init__(self):
        self.myStack = []
    ...
    def empty(self):   return len(self.myStack) == 0`
        },
        {
          ...
          insight: "💡 A better approach uses two stacks: one for incoming items and one for outgoing items. You only move elements between them when needed, which makes dequeues fast on average (amortized O(1)).",
          code: `class MyQueue:
    def __init__(self):
        self.myQueue = []
      ...
      problems: [
        {
          ...
          approach: "Instead of unecessary recursion (O(2ⁿ)), keep only the last two values and update in a loop. The Pythonic tuple swap a, b = b, a + b eliminates the need for a temp variable.",
          insight: "💡 Fibonacci is a common example for dynamic programming and memoization. This iterative version uses bottom-up DP with O(1) space because each new value only depends on the previous two, so we don’t need to store the full sequence.",
          code: `class Solution:
    def fib(self, n):
        a, b = 0, 1
        for _ in range(n):
        ...
        },
        {
          ...
          insight: "💡 This is a standard dynamic programming pattern: recognizing that the problem follows a known recurrence (like Fibonacci). The “staircase” wording is just a disguise for the same repeating subproblem structure.",
          code: `class Solution:
    def climbStairs(self, n):
        ...
        },
        {
          ...
          approach: "At each house, the optimal choice is: max(rob this house + best from two back, skip and take best from one back). Track only the previous two best value.",
          insight: "💡 Each step only depends on the previous two values, not the full history. That lets us replace the full O(n) table with just two variables.",
          code: `class Solution:
    def rob(self, nums):
        ...
        },
        {
          ...
          approach: "Recursive solution with caching (@cache). If we reach the target with no dice left, count it as 1 way. If either condition fails, return 0. Otherwise, try all k face values, sum the results, and take mod to keep numbers small.",
          insight: "💡 @cache stores results of function calls so we don’t recompute the same states. This turns an exponential recursion into a much faster solution by reusing previous work.",
          code: `class Solution:
    @cache
      ...
      problems: [
        {
          ...
          insight: "💡 A star graph has a central node that appears in every edge. Recognizing that structure lets you identify the answer immediately instead of computing degrees for all nodes.",
          code: `class Solution:
